  ...
  for dirEntry in dirList:
    for fileEntry in dirEntry[2]:
      if not fileEntry.endswith('~'):
        fn = os.path.join(dir, fileEntry)
        if (os.path.isfile(fn)):
          en = os.path.join(os.path.relpath(dirEntry[0], dir), fileEntry)
          print('Write %s to %s' % (fn, en))
          tzip.write(fn, en)
  tzip.close()
  return ret.getvalue()
...
for topName in os.listdir(Directory):
  if not pathContainsDot(topName):
    ...
    en = os.path.join(os.path.relpath(fn, Directory), topName)
    if (os.path.isdir(fn)):
      contents = zipDirectory(fn)
      print('Directory %s into %s' % (fn, en))
      zipout.writestr(en, contents)
    else:
      print('File %s into %s' % (fn, en))
      zipout.write(fn, en)
zipout.close()
